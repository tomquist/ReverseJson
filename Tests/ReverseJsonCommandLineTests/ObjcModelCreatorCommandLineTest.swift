...
class ObjcModelCreatorCommandLineTest: XCTestCase {
    ...
    func testAtomicFieldsFlag() {
        let modelCreator1 = ObjcModelCreator(args: ["-a"])
        let modelCreator2 = ObjcModelCreator(args: ["--atomic"])
        XCTAssertEqual(modelCreator1.atomic, true)
        XCTAssertEqual(modelCreator2.atomic, true)
    }
    
    func testMutableFieldsFlag() {
        let modelCreator1 = ObjcModelCreator(args: ["-m"])
        let modelCreator2 = ObjcModelCreator(args: ["--mutable"])
    ...
    func testPrefixOption() {
        let modelCreator1 = ObjcModelCreator(args: ["-p", "ABC"])
        let modelCreator2 = ObjcModelCreator(args: ["--prefix", "ABC"])
